...
export const useEmployeeListStore = defineStore('EmployeeListStore', {
  ...
  actions: {
    ...
    // 👉 Fetch employee logbook 
    fetchEmployeeLogbook(employeeId, date_string) { 
      const response = new Promise((resolve, reject) => {
        axios.get(`${API_BASE_URL}/logbook/employee/${employeeId}/${date_string}`).then(response => resolve(response)).catch(error => reject(error))
      })

      response.then(response => {this.employeeLogbook = response})
    },
